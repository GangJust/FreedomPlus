...
import io.github.xpler.core.bridge.ConstructorHook
import io.github.xpler.core.bridge.ConstructorHookImpl
import io.github.xpler.core.bridge.MethodHook
...
class KtXposedHelpers {
    ...
    companion object {
        ...
        /**
         * 返回模块版本名, 需要首先 [initModule] 调用
         */
        fun moduleVersionName(context: Context): String? {
            return modulePath.ifNotEmpty {
                KAppUtils.getApkPackageInfo(context, File(it))?.versionName
            }
        }

        /**
         * 返回模块版本号, 需要首先 [initModule] 调用
         */
        fun moduleVersionCode(context: Context): Long? {
            return modulePath.ifNotEmpty {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    KAppUtils.getApkPackageInfo(context, File(it))?.longVersionCode
                } else {
                    KAppUtils.getApkPackageInfo(context, File(it))?.versionCode?.toLong()
                }
            }
        }
         ...
         * 可以参考 [io.github.xpler.HookInit.handleLoadPackage] 的实现
         */
